<template>
  <div class="max-w-screen-lg w-full px-2 py-2 my-0 mx-auto">
    <div class="flex flex-wrap gap-1">
      <a :href="withBase(`FAQ#outline`)" class="simple-tag" style="text-decoration: none" @click="currentTag = null">
        <span>全部</span>
        <span class="pl-1"> {{ data.posts.length }} </span>
      </a>
      <a v-for="(tag, i) in tags" :key="i" :href="withBase(`FAQ?tag=${tag}#outline`)" class="simple-tag"
        style="text-decoration: none" @click="currentTag = tag">
        <span>{{ tag }}</span>
