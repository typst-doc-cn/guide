...
后台报错策略会影响是否向后台报告以上结果，以及是否抛出异常，且仅限于此——只要`pnpm run build`成功运行完，那么无论哪种策略，网页上显示的代码、编译结果、日志、Typst版本提示信息都完全相同。

报错策略可以每个代码块单独设置。

- 默认策略

  普通环境接受任何编译结果，CI 环境只接受 ok（遇到 warning 或 error 会抛出异常）。

  无论哪种环境，后台都报告日志（如有）。

- 用[语言标签](#支持的语言标签)专门指定`expect-warning`

  普通环境接受任何编译结果，CI 环境只接受 warning（遇到 ok 或 error 会抛出异常）。

  无论哪种环境，若结果为 warning，则后台不再报告日志，否则照常报告日志（如有）。

  网页上会增加“上例出现警告是正常现象”提示信息。
